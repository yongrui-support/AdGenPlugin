...
- 🚨🚨🚨 **暫存檔絕對別丟在 project root！很重要，講三次：別丟 root、別丟 root、別丟 root。** 🚨🚨🚨
  分兩類、各有歸處，用完清掉：
    - **生圖／瀏覽器流程的暫存**（下載 base64、截圖、dataURL 等）→ 使用者專案的 **`.browser/tmp/`**（見 generate-images）。
    - **Claude 自己做事的過程性暫存**（讀 JSON 寫的中間檔、dump、過程記錄等，**不是來自瀏覽器**）→ 專案的 **`tmp/` 資料夾**（已 gitignore）。
  - ⚠️ **再強調一次**：任何 `.txt`／`.json`／`.png` 中間檔，**只能**落在 `.browser/tmp/`（瀏覽器流程）或 `tmp/`（Claude 過程性），**不准散在專案根**（會被看到、弄亂版控）。MCP 的 `browser_evaluate`／`screenshot` 給 `filename` 時務必帶 `.browser/tmp/` 前綴，別用裸檔名（裸檔名會落到 root）。
  - ⚠️ **第三次提醒**：收工前清掉自己產的暫存（`.browser/tmp/` 由 generate-images §5 清空；`tmp/` 的過程檔用完即刪）。**檢查 project root 是否乾淨**再回報完成。
...
- **並發鐵則**（threaded=True 多請求並行，且 `generate-images` 是**另一行程**寫同一份 JSON）：凡「讀改寫」creatives JSON 一律包 `_JSON_LOCK`（擋 server 內多執行緒互蓋；含讀取時 schema 遷移的寫回），且**寫回前重讀最新檔、以 uid 找回該組**——拿舊快照整份覆寫會洗掉別人剛寫入的結果。`PUT` 的 `uid`/`images` 一律以磁碟為準（勿改成信任前端）。`_JSON_LOCK` **管不到跨行程的 `generate-images` 回寫**——那靠兩份 skill 都遵守「append/寫回前重讀最新檔＋以 uid 比對＋原子寫入（.tmp→os.replace）」協調（見 generate-images §4 / generate-creatives §5）。
