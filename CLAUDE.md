...
- `migrations.py` — 批次 JSON 的 **schema 版本化遷移**（檔案即演進史）：`SCHEMA_VERSION` + 逐版增量遷移函式；server 讀取時惰性套用（缺版號視為 0、逐版補課、函式需冪等）。改 schema = 版號 +1、加 `_migrate_N_to_N+1`、SKILL.md 同步。
...
- `data/creatives/<id>.json` — `generate-creatives` skill 的產出（gitignore）；看板編輯/刪除會回寫。每組有穩定 `uid`。**brief = 發想時的輸入快照**（`default_offer`/`default_aspect` 是預設值，卡片的 content / 生圖比例可蓋過，分歧是正常演化）。schema 見該 skill 的 SKILL.md，版號機制見 `migrations.py`。
