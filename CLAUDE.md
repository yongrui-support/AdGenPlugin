...
- **暫存檔別散在專案根**——分兩類各有歸處，用完清掉：
    - **生圖／瀏覽器流程的暫存**（下載 base64、截圖等）→ 使用者專案的 `.browser/tmp/`（見 generate-images）。
    - **Claude 自己做事的過程性暫存**（讀 JSON 寫的中間檔、dump、過程記錄等，**不是來自瀏覽器**）→ 專案的 **`tmp/` 資料夾**（已 gitignore）。**別丟在 project root**（會被看到、弄亂）。
