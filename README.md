...
主視覺**不打 OpenAI API（省錢）**——改由 **`generate-images` skill** 用 **CDP 接管你已登入的 ChatGPT**
（吃訂閱、還能讓 Claude 逐張回讀對標、迭代到過關）生圖，存到 `data/images/` 並**回寫 JSON**。
...
| skill | 用途 |
|---|---|
| `/ad-generator:setup` | **第一次先跑這個**：偵測 OS → macOS 裝 Homebrew、Windows 裝 Scoop → 裝 uv → 安裝相依；要生圖再選用備 **CDP 瀏覽器**（Node/npx + 啟動腳本 + 登入 ChatGPT） |
...
1. `/ad-generator:setup`（首次，裝好 uv 與相依；要生圖再備 CDP 瀏覽器）。
...
生圖需要 CDP 瀏覽器環境（Playwright MCP 已內建在 `.mcp.json`）：跑 `setup` 第 5 步、或手動執行
`templates/launch-chrome-cdp.{bat,sh}`、登入 ChatGPT、再啟動 Claude Code。細節見
[docs/cdp-基本觀念.md](docs/cdp-基本觀念.md) 與 `skills/generate-images/SKILL.md`。
...
│   └── setup/SKILL.md                環境安裝（brew/scoop → uv；選用備 CDP 瀏覽器）
├── templates/
│   └── launch-chrome-cdp.{bat,sh}    CDP Chrome 啟動腳本範本（setup 複製到使用者專案 .browser/）
├── docs/
│   └── cdp-基本觀念.md               CDP 入門（小白向）
