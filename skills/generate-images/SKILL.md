...
`content`（圖中文字）、`composition_prompt`（構圖，含 `{{content.欄位}}` 佔位）、`aspect`（比例）、`materials`（素材名）、`uid`、**`pipeline_mode`**（生圖路徑：`chatgpt`／`both`／`gemini`，預設 `chatgpt`；決定走哪條，見 §2）。
**並讀該批品牌的設計準則 `data/materials/<品牌>/design.md`（若有）**（品牌＝`brief.brand_name`）——它是生圖與回讀驗收的準則（見 §3）；沒有就略過。

## 2. 逐組產圖（多分頁並行）

**先看每組 `pipeline_mode` 決定走哪條路徑**：
- **`chatgpt`（預設）** → 本節的 ChatGPT 並行流程。
- **`both`** → 先 Gemini 打底、再 GPT 改圖收尾（見 §2B）。
- **`gemini`** → 只用 Gemini（§2B 的前半）。

> **Pre-flight（產圖前先做）**：若這批**有任何組是 `both`／`gemini`**，除了 ChatGPT，還要**請使用者把 `gemini.google.com` 也開好並登入好**（在同一台 9222 CDP Chrome 開一個分頁登入 Google，登一次就好）。**兩邊都備妥再開始生圖**；只有 `chatgpt` 組就不必開 Gemini。

下面先講 `chatgpt` 路徑（多數組走這條）；`both`／`gemini` 見 §2B。
...
## 2B. Gemini 路徑（`pipeline_mode` = `both` / `gemini`）——物理/方向題

這條解 GPT 的「世界觀」弱點：**物體方向／朝向／倒下↔站起／誰看著誰／空間相對位置**，Gemini 的空間物理較準（實測對「倒下↔站直」這類難題明顯好過 GPT 單跑）。但 Gemini 的 **logo 合成／中文字／整體質感較弱**——所以 `both` 用 Gemini 打底、再交 GPT 收尾。

1. **Gemini 打底**（`browser_*` 操作）：`browser_navigate` 到 `gemini.google.com/app` → 開新對話 → 點「上傳與工具」→「檔案」開檔案選擇器 → `browser_file_upload` 附**該組 `materials` 的圖**（+ 依 design.md 準則）→ 在輸入框貼**該組同一份 prompt**（`{{content.欄位}}` 已換成實際字）→ Enter，等生成。
2. **抓 Gemini 圖**（跨網域 `fetch` 會被 CORS 擋，**改用 canvas**——Gemini 圖未被污染、canvas 讀得到）：
   ...
   - **`gemini` 模式到此即成品**：這張 PNG 就是最終圖、**不在 ChatGPT**——**§4 別再去 ChatGPT fetch**，直接把這個 PNG 搬進 `data/images/<批次>/<uid>.png`、驗證、回寫即可。仍走 §3 回讀驗收（不過就改 prompt 在 Gemini 重生）。
3. **`both` 模式——GPT 改圖收尾**：把上一步的 Gemini PNG ＋**正確的 logo 素材**一起傳進 **ChatGPT**（`Control+u` 附兩張），用「改圖模式」prompt：
   > 「**以附圖一為基底改圖**，**完整保留**構圖、物體姿態與方向、文字內容與位置；只做兩件：① 換上附圖二的正確 logo（忠實配色、不變形、不留佔位框）；② 整體質感／光影／合成精緻度提升到一線品牌級。」

   送出、等生成 → 進 §3 回讀驗收。需要時再追一輪自評精修（CTA／徽章質感等，見 §3 自評機制）→ §4 存檔。

> 為什麼這樣分：**Gemini 物理/方向強、GPT 合成/中文字/質感強**——`both` 各取所長。`both`／`gemini` 同樣吃 §3 的「功能性必過＋design.md＋審美」驗收，不過不收。

> **`gemini`-only 不適合「要放 logo」的組**：Gemini logo 合成弱、又沒有 GPT 收尾救，而 §3 把「logo 配色/位置正確」列必過 → 可能反覆生不過。**要 logo 的組請用 `both`**（GPT 收尾補 logo）；`gemini`-only 留給「不放 logo／logo 非關鍵」的組。

> **混合批次編排（別讓 `both` 退化成序列）**：一批常是 `chatgpt`（並行）＋ `both`／`gemini`（§2B）混合。仍守 §2 的「先全送、再統一等」：**① 同時點火——`chatgpt` 組在 ChatGPT 多分頁、`both`/`gemini` 組在 Gemini 多分頁，各自全部送出 → ② 統一等一次 → ③ 收圖（ChatGPT 那批 fetch、Gemini 那批 canvas）→ ④ `both` 組再把各自的 Gemini 底圖＋logo 一次丟進 ChatGPT 多分頁並行做 GPT 收尾 → 再統一等、收。** 別「一組從頭做到尾（Gemini 等完→GPT 等完）再做下一組」，那樣 `both` 一多就超慢。
...
1. **存圖**：`data/images/<批次id>/<uid>.png`（**cwd 相對＝使用者專案**），`uid = uuid4().hex[:12]`（每張新 uid，唯一命名；資料夾給人翻、uid 給程式引用）。下載方式：聊天分頁內 `fetch` 圖 URL → base64 → 存檔（簽名 URL 需登入 cookie，PowerShell 直接抓會 403）。**base64 中間檔落在使用者專案的 `.browser/tmp/`**：**下載前先 `mkdir -p "${CLAUDE_PROJECT_DIR:-$(pwd)}/.browser/tmp"` 確保該夾存在**（`browser_evaluate` 不一定會自動建父層，第一次寫會失敗）。然後 `browser_evaluate` 的 `filename` 給**絕對路徑** `${CLAUDE_PROJECT_DIR:-$(pwd)}/.browser/tmp/dl_<uid>.txt`（即 §頂規約那個暫存夾；別給裸檔名——預設會散在使用者專案根）。回寫小腳本再從這個絕對路徑讀回、解出 PNG 存到使用者專案 `data/images/`（`<批次id>` ＝該批 creatives JSON 的 id，即檔名 `<id>.json` 的 `<id>`）。
   - **`gemini`-only 組例外**：最終 PNG 在 §2B step 2 已用 canvas 抓進 `.browser/tmp/gemini_<uid>.txt`→PNG——**不必再 fetch ChatGPT**，直接把那張 PNG 搬到上述 `data/images/<批次>/<uid>.png`（下面的有效性驗證照做）。`both` 組的最終圖在 ChatGPT（GPT 收尾那張），仍照一般 fetch。
