...
2. **開一個 `Agent` 呼叫**，`subagent_type: "claude"`（獨立 context、能 Read PNG）。**一次驗整批**（不是每張開一個 subagent）。給它的 prompt 必須包含：
...
3. **主 agent 根據 subagent 結果處理**：
   ...
   - **整批最多 2 輪**；2 輪仍不過分兩種收尾：**文字／優惠數字錯** → 廢稿、**不存進相簿**、極大聲標「文字錯誤、勿用」，請使用者改 `content` 或換 `ai_platform`／`model`／`thinking_effort`；**美感／質感差** → 存目前最好一版、標「待人工」。都回報使用者。
4. **整批 PASS 才能寫 JSON**（§4）。**未經 subagent 驗就直接寫檔＝違規**——這是業主明確要求的雙層 gate，目的是擋掉主 agent 的自我合理化。
...
- **功能性「一票否決」（這幾項比美感更致命，不過＝不收、別只看好不好看）**：
  ...
  2. **比例符合該組 `aspect`**（不符＝不過；若 §3.1 的 2 輪複驗仍拿不到目標比例 → 取最接近版、必要時裁切、回報該組比例未達，§2 已知此法命中率較低）。
...
**▶ 請模型自評再優化（可選；與 §3.1 subagent 驗收不同——此步是 GPT 生成端的靈感優化、求建議；§3.1 是獨立 subagent 審查端的守關、判過／不過。兩者互補不衝突）**：只在 subagent 已給 PASS 卻仍覺得「真的很瞎、很不到位」時才做（一般 PASS 的不用每張都來，省時間）。在同一個對話直接問 GPT「這張你覺得哪裡可以更好？構圖/光影/敘事/字體排版/品牌一致性，給我具體可執行的調整」。
