...
單帳號 Plus **一次開到 5 分頁並行**（5 張一起送、再統一等，省時間）；再往上可試但注意時段額度，被限速就分批（每批仍是「先全送、再統一等」）。
...
- **品牌 `design.md`（§1 已讀，若有）是「必過」驗收項**：逐條對照（配色／字體／版面／風格／必避免的元素），**只要不符合就改 prompt、跟 GPT 調到符合為止**，不符不收；它與下面的通用審美衝突時，**以 `design.md` 為準**。
